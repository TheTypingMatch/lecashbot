...
const formatTime = ms => {
    let seconds: string | number = Math.round((ms / 1000) % 60);
    let minutes: string | number = Math.round((ms / (1000 * 60)) % 60);
    let hours: string | number = Math.round((ms / (1000 * 60 * 60)) % 24);
    const days: string | number = Math.round((ms / (1000 * 60 * 60 * 24)) % 365);
