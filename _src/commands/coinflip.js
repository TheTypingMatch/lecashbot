...
const sendReward = (msg, user, client, embed, recordEmbed) => {
    ...
    const reward = Math.round(100 * (3 ** (coinflipStreak - 1)) + (coinflipStreak * 150));
    const streakChance = Math.round((100 / (2 ** (coinflipStreak + 1))) * 100) / 100;
    const cost = (coinflipStreak) ? Math.round(100 * (2 ** coinflipStreak)) : 0;
    ...
    const nextCost = 100 * (2 ** (coinflipStreak + 1));
    const nextReward = 100 * (3 ** coinflipStreak) + ((coinflipStreak + 1) * 150);
    const description = {
